...
describe('reflowSkillCrt', () => {
  ...
  it('stacks translated left-column then right-column skills in hotkey order', () => {
    const translated = [
      '  스킬        레벨    스킬        레벨',
      `  ${'a - 전투 기술    +0'.padEnd(19)}${L('c - 주문 시전   +11')}`,
      `  ${'b - 회피술       +1'.padEnd(19)}${L('d - 주술        +11')}`,
    ]
    const out = reflowSkillCrt(translated).map(text)
    const skills = out
      .map(t => (/^\s*([a-z]) [+\-*] (\S+)/.exec(t)))
      .filter(Boolean)
      .map(m => `${m![1]}:${m![2]}`)
    expect(skills).toEqual(['a:전투', 'b:회피술', 'c:주문', 'd:주술'])
  })
  ...
  describe('distributed training (no hotkeys)', () => {
    ...
    it('stacks translated rows by the sign anchor', () => {
      const translatedBare = [
        '  스킬        레벨    스킬        레벨',
        `  ${'+ 전투 기술     +8'.padEnd(18)}${L('+ 주문 시전    +8')}`,
        `  ${'+ 회피술        +8'.padEnd(18)}${L('+ 주술         +6')}`,
      ]
      const out = reflowSkillCrt(translatedBare).map(text)
      const skills = out
        .map(t => (/^\s*\+ (\S+)/.exec(t)))
        .filter(Boolean)
        .map(m => m![1])
      expect(skills).toEqual(['전투', '회피술', '주문', '주술'])
    })
